...
asio::awaitable<void> conversation_example(openai::Client& client) {
    ...
    try {
        ...
        if (!response1.has_value() || response1.value().choices.empty()) {
            fmt::print("Error: {}\n", response1.has_value() ? "No choices returned" : response1.error().to_string());
            co_return;
        }
        
        fmt::print("Assistant: {}\n\n", response1.value().choices[0].message.content);
        
        // Add assistant's response to history
        request.messages.push_back(response1.value().choices[0].message);
        ...
        if (response2.has_value() && !response2.value().choices.empty()) {
            fmt::print("Assistant: {}\n\n", response2.value().choices[0].message.content);
            
            fmt::print("Total conversation tokens: {}\n", 
                response1.value().usage.total_tokens + response2.value().usage.total_tokens);
        }
